...
import { get_withdrawal_history } from "../../APIs/commonAPIsStructure";
import moment from "moment";
const WithdrawHistory = ({ navigation, route }) => {
  ...
  const withdrawal_history = async () => {
    try {
      ...
      const history = await get_withdrawal_history(
        parseToken?.token,
      );
      console.log('-=-=-=--history=-=-=-history=-=-=-1212=-=-=-', history);
      setHistoryData(history?.data?.withdrawals)
    } catch (error) {
    ...
    <View style={styles.itemContainer}>
      ...
      <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
        ...
        <View style={{ flexDirection: "row" }}>
          ...
          <Text style={[styles.text,{ textAlign:"right"}]}>{`${item.orderId.substring(0,10)}***${item.orderId.substr(item.orderId.length-10)}`}</Text>
        </View>
      </View>
      <View
        style={{
      ...
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerText}>
            {t("historyScreens.withdrawScreen.withdraw")}
          </Text>
          <View />
        </View>
